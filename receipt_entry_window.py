...
class Ui_MainWindow_receipt_entry(object):
    def setupUi(self, MainWindow_receipt_entry):
        ...
        MainWindow_receipt_entry.setWindowFlags(QtCore.Qt.WindowCloseButtonHint | QtCore.Qt.WindowMinimizeButtonHint)

        self.assign()

    def retranslateUi(self, MainWindow_receipt_entry):
        _translate = QtCore.QCoreApplication.translate
        MainWindow_receipt_entry.setWindowTitle(_translate("MainWindow_receipt_entry", "Voucher Entry"))
    ...
    def assign(self):
        ...
        self.comboBox_member_id.setEditable(True)
        self.comboBox_member_id.activated.connect(lambda: self.get_member_name_by_id())
        ...
        for i in ids:
            self.comboBox_member_id.addItem(str(i))
    ...
    def get_member_name_by_id(self):
        i = self.comboBox_member_id.currentText()
        conn = sqlite3.connect("RSKT.db")
        cur = conn.cursor()
        sql = "SELECT name FROM member WHERE id = " + i
        try:
            cur.execute(sql)
            item = cur.fetchone()
            conn.commit()
            self.lineEdit_member_name.setText(item[0])
        except Exception as e:
            message = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Critical, "error", f"Member of ID {str(i)} does not exist")
            message.exec_()
        finally:
            conn.close()
    ...
    def save(self):
        ...
        try:
            ...
            unit_amount = math.ceil(total_amount / count)

            paid_fors.append(start_paid_for.toString("MM/yyyy"))
            for i in range(count):
                start_paid_for = start_paid_for.addMonths(1)
                paid_fors.append(start_paid_for.toString("MM/yyyy"))
        ...
        try:
            for i in range(int(count)):
                cur.execute(sql,{
                    ...
                    "paid_for": paid_fors[i],
                    "payment_method": payment_method,
                    "reference": reference,
                    "remarks": remarks
                })
                conn.commit()
            message = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Information, "Success", "Receipt Entry Successful")
            message.exec_()
        except Exception as e:
